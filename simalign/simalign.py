        ...
        if self.emb_model is not None:
            with torch.no_grad():
                ...
                if self.layer >= len(hidden):
                    raise ValueError(
                        f"Specified to take embeddings from layer {self.layer},"
                        " but model has only {len(hidden)} layers."
                    )
...
class SentenceAligner(object):
    ...
    def apply_distortion(sim_matrix: np.ndarray, ratio: float = 0.5) -> np.ndarray:
        ...
        pos_x = np.array(
            [
                [y / float(shape[1] - 1) for y in range(shape[1])]
                for x in range(shape[0])
            ]
        )
        pos_y = np.array(
            [
                [x / float(shape[0] - 1) for x in range(shape[0])]
    ...
    def get_word_aligns(
        ...
        bpe_lists = [
            [bpe for w in sent for bpe in w] for sent in [l1_tokens, l2_tokens]
        ]
