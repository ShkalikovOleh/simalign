...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="The layer the embeddings should be taken from.",
    )
    ...
    # --------------------------------------------------------
    embed_loader = EmbeddingLoader(model=args.model, device=device, layer=args.layer)
    ...
    for path in original_paths:
        ...
        corpus = [
            regex.sub("\\p{C}+", "", regex.sub("\\p{Separator}+", " ", l)).strip()
            for l in corpus
        ]
    ...
    for sent_id in range(len(words_tokens)):
        ...
        b2w_map_pair = [
            [i for i, w in enumerate(sent) for bpe in w]
    ...
    out_f = {
        ext: open("{}.{}".format(args.output, ext), "w") for ext in matching_methods
    }
    if args.log:
    ...
    for batch_id, batch_sentences in enumerate(tqdm(data_loader)):
        ...
        for in_batch_id, sent_id in enumerate(batch_sentences[0].numpy()):
            ...
            raw_scores = {
                x: collections.defaultdict(lambda: []) for x in matching_methods
            }
            b2w_scores = {
                x: collections.defaultdict(lambda: []) for x in matching_methods
            }
                    ...
                    if not args.add_probs:
                        ...
                        out_f[ext].write(
                            ...
                            + " ".join(
                                sorted(
                                    [
                                        f"{p}-{str(round(np.mean(vals), 3))[1:]}"
                                        for p, vals in raw_scores[ext].items()
                                    ]
                                )
                            )
                            + "\n"
                        ...
                        out_f[ext].write(
                            ...
                            + " ".join(
                                sorted(
                                    [
                                        f"{p}-{str(round(np.mean(vals), 3))[1:]}"
                                        for p, vals in b2w_scores[ext].items()
                                    ]
                                )
                            )
                            + "\n"
